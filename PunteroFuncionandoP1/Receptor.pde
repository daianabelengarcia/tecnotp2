class Receptor { //<>//
  ...
  int tiempo_para_borrar = -10; // Tiempo que tarda en desaparecer y reaparecer el Blob.
  boolean hayBlobEnPantalla = false; // Variable para detectar si hay un blob en la pantalla
  ...
  void actualizar(ArrayList <OscMessage> mensajes) {
    ...
    hayBlobEnPantalla = false; // Reiniciar la detección en cada ciclo
    ...
    while (mensajes.size() > 0) { // Mientras hay mensajes en el buffer
      ...
      if (m.addrPattern().equals("/bblobtracker/blobs")) { // si el mensaje tiene la etiqueta (address) "/bblobtracke/blobs"
        ...
        int id = (int) m.get(1).floatValue(); // le pido el ID a cada blob del mensaje (este caso la edad)
        ...
        for (int i=0; i<blobs.size(); i++) { // recorro la lista de blobs del repector
          ...
          if (b.id == id) { // si el blob del mensaje ya estaba en mi lista de blobs
            ...
            hayBlobEnPantalla = true; // Marcar que hay un blob en la pantalla
            break; // salgo del siclo for para que deje de buscar
          }
        }
        if (!encontrado) { 
          Blob nb = new Blob(1);   // creo un NUEVO blob
